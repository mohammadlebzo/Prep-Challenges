    ...
    for (let i = 0; i < arr.length; i++) {
        if (arr[i].yearsOfExperience > 1) {
            if (arr[i].lastName === null) {
                outArr.push({ fullName: `${arr[i].firstName}`, tech: `${arr[i].tech}` });
            }
            else {
                outArr.push({ fullName: `${arr[i].firstName} ${arr[i].lastName}`, tech: `${arr[i].tech}` });
            }
        }
    }
    return outArr;
    ...
    for (let i = 0; i < data.grades.length; i++) {
        for (let j = 0; j < data.grades[i].classes.length; j++) {
            ...
            for (let k = 0; k < data.grades[i].classes[j].classScores.length; k++) {
                tempSum += data.grades[i].classes[j].classScores[k];
            }
            data.grades[i].classes[j].avg = Math.floor(tempSum / data.grades[i].classes[j].classScores.length);
        }
    }
    return data;
};
