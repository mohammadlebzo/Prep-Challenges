...
const recursionPattern = (int1, int2) => {
    ...
    if (int1 < 0) {
        arr = int1;
        let temp = [arr];
        for (let i = 1; i <= Math.ceil((int2 / 2) + 1); i++) {
            temp.push(arr + (int2 * i));
        }
        return temp;
    }
    arr = int1;
    return (arr + ',' + recursionPattern(int1 - int2, int2)).split(',').map(item => parseInt(item));
}
// -------------------------------------------------------------------------------------------------------
...
const filterLinks = (str) => {
    // write your code here
    let regex = /(www.*.com)|(www.*.org)|(www.*.net)/gi;
    if (regex.test(str)) {
...
const isPalindrome = (str) => {
    // write your code here
    let tempStr = str.split(',').join('').split(' ').join('').split(':').join('').toLowerCase();
    let reversedTempStr = tempStr.split('').reverse().join('');
    if (str === '') {
        return true;
    }
    if (tempStr === reversedTempStr) {
        return true;
    }
    else {
        return false;
